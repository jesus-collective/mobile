...
const completeBillingScreen = (coupon) => {
  cy.contains("Centered-Set Church", { timeout: 30000 })
...
describe("Create User", () => {
  sizes.forEach((size) => {
    it("Size - " + size, () => {
      ...
      cy.visit("/auth/signin?brand=oneStory&joinedProduct=product_MarkBaker").then(() => {
...
describe("Create User + Org", () => {
  sizes.forEach((size) => {
    it("Size - " + size, () => {
      ...
      cy.visit("/auth/signin?brand=oneStory&joinedProduct=product_MarkBaker")
        .then(() => {
          TestHelper.DeleteUser(user, "TestTest#1")
        })
        .contains("Sign In")
