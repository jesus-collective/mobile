...
class MyConfirmSignUpImpl extends React.Component<Props, State> {
    ...
    handleEnter(actions: any, keyEvent: NativeSyntheticEvent<TextInputKeyPressEventData>): void {
        if (keyEvent.nativeEvent.key === 'Enter')
            this.handleConfirmSignUp(actions);
    }
                                ...
                                (<View style={{ width: "100%", left: 0, top: 0, height: "100%" }}>
                                    ...
                                    <View style={this.styles.style.authView2}>
                                        ...
                                        <View style={this.styles.style.confirmationCodeWrapper}>
                                            <TextInput textContentType="oneTimeCode" keyboardType="number-pad" onKeyPress={(e) => this.handleEnter(actions, e)} placeholder="One-time security code" value={this.state.code} onChange={e => this.setState({ code: e.nativeEvent.text })} style={{ borderBottomWidth: 1, borderColor: "#00000020", marginBottom: '1.4%', marginRight: 30, width: '100%', paddingTop: 10, paddingRight: 10, paddingBottom: 10, paddingLeft: 5, fontFamily: 'Graphik-Regular-App', fontSize: 18, lineHeight: 24 }}></TextInput>
                                            <JCButton buttonType={ButtonTypes.SolidSignIn2} onPress={() => this.handleConfirmSignUp(actions)}>{this.state.sendingData ? <ActivityIndicator animating color="#333333" /> : 'Submit'}</JCButton>
                                        </View>
