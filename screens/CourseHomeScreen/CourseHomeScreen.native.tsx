...
export default class CourseHomeScreenImpl extends JCComponent<
  Props,
  CourseState
> {
  constructor(props: Props) {
    ...
    this.state = {
      ...
      currentScreen: props.route.params.screen
  ...
  updateBackOfficeStaff = async (value: any): Promise<void> => {
    ...
    const add = value.filter(
      (x) =>
        !this.state.courseData.backOfficeStaff.items
          .map((z) => z.userID)
          .includes(x.id)
    );
  ...
  updateInstructors = async (value: any): Promise<void> => {
    ...
    const add = value.filter(
      (x) =>
        !this.state.courseData.instructors.items
          .map((z) => z.userID)
          .includes(x.id)
    );
    // const delTriadID= this.state.courseData.triads.items[index].users.items.map((item)=>{del.contains(item.})
    ...
    add.map(async (item) => {
      ...
      try {
        ...
        const temp = this.state.courseData;
        temp.instructors.items.push(
          createCourseInstructors.data.createCourseInstructors
        );
        console.log(temp);
        this.setState({ courseData: temp });
    ...
    del.map(async (item) => {
      try {
        ...
        console.log(createCourseTriadUsers);
        const temp = this.state.courseData;
        temp.instructors.items = temp.instructors.users.items.filter(
          (user) => user.id !== item.id
        );
        console.log(temp);
        this.setState({ courseData: temp });
  ...
  updateTriadUsers = async (index: number, value: any): Promise<void> => {
    ...
    const add = value.filter(
      (x) =>
        !this.state.courseData.triads.items[index].users.items
          .map((z) => z.userID)
          .includes(x.id)
    );
    // const delTriadID= this.state.courseData.triads.items[index].users.items.map((item)=>{del.contains(item.})
    ...
    del.map(async (item) => {
      try {
        ...
        console.log(createCourseTriadUsers);
        const temp = this.state.courseData;
        temp.triads.items[index].users.items = temp.triads.items[
          index
        ].users.items.filter((user) => user.id !== item.id);
        console.log(temp);
        this.setState({ courseData: temp });
      } catch (createCourseTriadUsers) {
        console.log(createCourseTriadUsers);
  ...
  myCourseGroups = (): { all: any[]; cohort: any[]; completeTriad: any[] } => {
    const z: [
      { cohort; completeTriad }
    ] = this.state.courseData?.triads?.items.map((item) => {
      let triadTemp = [],
        coachTemp = [],
        cohortTemp = [];
      let completeTriad = null;
      if (
        item.users.items.filter((user) => user.userID == this.state.currentUser)
          .length > 0 ||
        item.coaches.items.filter(
          (user) => user.userID == this.state.currentUser
        ).length > 0
      ) {
        ...
        cohortTemp = cohortTemp.concat(
          item.coaches.items.map((item) => item.user)
        );
    ...
    const allWithDuplicates: Array<any> = [
      ...fromTriads,
      ...instructors,
      ...backOfficeStaff,
    ];
  ...
  updateTriadCoaches = async (index: number, value: any): Promise<void> => {
    ...
    const add = value.filter(
      (x) =>
        !this.state.courseData.triads.items[index].coaches.items
          .map((z) => z.userID)
          .includes(x.id)
    );
    // const delTriadID= this.state.courseData.triads.items[index].users.items.map((item)=>{del.contains(item.})
  ...
  createLesson = async (): Promise<void> => {
    const resource: CreateCourseLessonInput = {
      ...
      lesson:
        this.state.courseData.courseWeeks.items[this.state.activeWeek].lessons
          .items.length + 1,
      //time: "",
      // description: JSON.stringify(convertToRaw(EditorState.createEmpty().getCurrentContent())),
      courseWeekID: this.state.courseData.courseWeeks.items[
  ...
  updateLesson = async (
    ...
    try {
      ...
      const updateWeek: any = await API.graphql({
        ...
        variables: {
          input: {
            id: this.state.courseData.courseWeeks.items[week].lessons.items[
              lesson
            ].id,
            [item]: value,
          },
