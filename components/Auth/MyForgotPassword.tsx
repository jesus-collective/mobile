...
class MyForgotPassword extends React.Component<Props, State> {
    ...
    async resetPass(): Promise<void> {
        try {
            ...
            await Auth.forgotPasswordSubmit(this.state.email, this.state.code, this.state.newPass)
            ...
            this.props.authState === 'forgotPassword' ?
                (<View style={{ width: "100%", left: 0, top: 0, height: "100%" }}>
                    ...
                    {!this.state.codeSent ? <View style={this.styles.style.authView2}>
                        <Text style={{ width: "100%", marginBottom: '5.55%', fontFamily: 'Graphik-Regular-App', fontWeight: 'bold', fontSize: 22, lineHeight: 30 }}>Reset your password</Text>
                        ...
                        <TouchableOpacity onPress={() => this.setState({ codeSent: true })}>
                        ...
                        : <View style={this.styles.style.authView2}>
                            <Text style={{ width: "100%", marginBottom: '5.55%', fontFamily: 'Graphik-Regular-App', fontWeight: 'bold', fontSize: 22, lineHeight: 30 }}>Reset your password</Text>
                            <TextInput placeholder="Enter your email" value={this.state.email} onChange={e => this.setState({ email: e.nativeEvent.text })} secureTextEntry={false} style={{ borderBottomWidth: 1, borderColor: "#00000020", width: "100%", marginBottom: '1.4%', paddingTop: 10, paddingRight: 10, paddingBottom: 10, paddingLeft: 5, fontFamily: 'Graphik-Regular-App', fontSize: 20, lineHeight: 28 }}></TextInput>
                            <TextInput placeholder="One-time security code" value={this.state.code} onChange={e => this.setState({ code: e.nativeEvent.text })} secureTextEntry={false} style={{ borderBottomWidth: 1, borderColor: "#00000020", width: "100%", marginBottom: '5.55%', paddingTop: 10, paddingRight: 10, paddingBottom: 10, paddingLeft: 5, fontFamily: 'Graphik-Regular-App', fontSize: 20, lineHeight: 28 }}></TextInput>
                            <View style={{ display: 'flex', flexDirection: 'row', marginBottom: '5.55%', }}>
