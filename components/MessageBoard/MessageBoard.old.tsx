  ...
  handlePressReply(item: Message | Reply) {
    if (item) {
      ...
      if ("replies" in item) {
        ...
        console.log("#2 there are replies", peopleInThread, "item.messageId", item.id)
        this.setState({ replyToId: item.id })
      } else {
        console.log("#2 there are no replies", peopleInThread, "item.messageId", item.messageId)
        this.setState({ replyToId: item.messageId })
      }
      console.log("#3 replyToWho:", peopleInThread, "replyToRoomId", item.roomId ?? "")
