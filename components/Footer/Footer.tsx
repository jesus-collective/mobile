          ...
          subMenu: [
            {
              ...
              linkTo: "EventsScreen",
              navProps: { mine: true },
              webLink: `${window.location.origin}/app/events?mine=true`,
            },
          ...
          subMenu: [
            {
              ...
              linkTo: "GroupsScreen",
              navProps: { mine: true },
              webLink: `${window.location.origin}/app/groups?mine=true`,
            },
            { name: "Recommended", linkTo: null },
          ],
        }
          ...
          subMenu: [
            {
              ...
              navProps: { create: false, id: constants["SETTING_KY_GROUP_ID"] },
              webLink: `${window.location.origin}/app/resource?create=false&id=${constants["SETTING_KY_GROUP_ID"]}`,
            },
  ...
  open = async (obj: SubMenuItem): Promise<void> => {
    if (Platform.OS === "web" && obj.webLink) {
      ...
      try {
        ...
        if (supported) await Linking.openURL(obj.webLink)
      } catch (e) {
        console.error(e)
      }
    } else {
        ...
        <Body style={footerStyles.footerBodyContainer}>
          <Body style={footerStyles.footerInnerBodyContainer}>
            <Button
              transparent
              onPress={() => {
                this.open({ name: "Home", linkTo: "HomeScreen", navProps: {} })
              }}
            >
              <Image style={footerStyles.logo} source={require("./icon.png")} />
            </Button>
            <Text style={footerStyles.copyRightText}>
              &copy; {new Date().getFullYear()} Jesus Collective. All Rights Reserved.
            </Text>
          </Body>
          {this.menu.map((item) => {
            if (item != null)
