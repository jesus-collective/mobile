...
class ResourceContent extends JCComponent<Props, State> {
  ...
  componentDidMount(): void {
    ...
    if (w >= 1280) this.setState({ rowLength: 4, numberOfVideos: 8, cardHeight: 200 + w * 0.09 })
    else if (w < 769) this.setState({ rowLength: 1, numberOfVideos: 4 })
    else this.setState({ rowLength: 3, numberOfVideos: 6, cardHeight: 200 + w * 0.12 })
  }

  componentDidUpdate(prevProps: Props): void {
    if (prevProps.currentResource !== this.props.currentResource) {
      this.state.rowLength === 3
        ? this.setState({ numberOfVideos: 6 })
        : this.setState({ numberOfVideos: 8 })
    }
  }

  renderSeriesMobile(state: ResourceState, actions: ResourceActions): React.ReactNode {
    const moreVideosLength =
      state.resourceData.resources.items[state.currentResource].series.items.length - 3
    return (
      <Container
        style={{
          display: "flex",
      ...
      >
        <Container style={this.styles.style.resourceContentLeftContainer}>
          ...
          <List>
            {state.resourceData.resources.items[state.currentResource].series.items
              .slice(0, 3)
              .map((series, index: number) => {
                ...
                return (
                  <ListItem key={series.id} style={{ borderBottomWidth: 0 }}>
                    <Card style={this.styles.style.resourceContentCurrentSeriesCard}>
                      {state.isEditable ? (
                        <CardItem>
                          ...
                          <JCButton
                            buttonType={ButtonTypes.TransparentNoPadding}
                            onPress={() => {
                              actions.deleteSeries(state.currentResource, index)
                            }}
                          >
                      ...
                      >
                        <CardItem
                          style={this.styles.style.resourceContentCurrentSeriesIframeContainer}
                        >
                        ...
                        >
                          <EditableText
                            onChange={(val) => {
                              actions.updateSeries(state.currentResource, index, "title", val)
                            }}
                            multiline={false}
                        ...
                        >
                          <EditableText
                            onChange={(val) => {
                              actions.updateSeries(state.currentResource, index, "description", val)
                            }}
                            multiline={true}
                        ...
                        >
                          <JCButton
                            ...
                            onPress={() => actions.changeSeries(index)}
                          >
                            Learn More
                          </JCButton>
                        </CardItem>
          ...
          <List style={{ height: "100%" }}>
            {state.resourceData.resources.items[state.currentResource].series.items
              .slice(3, this.state.numberOfVideos + 4)
              .map((series, index: number) => {
                const thumbnailIndex = this.findFirstEpisode(series.episodes.items)
                index += 3
                return (
                  <ListItem key={series.id} style={{ borderBottomWidth: 0 }}>
                    <Card style={this.styles.style.resourceContentCurrentSeriesCard}>
                      ...
                      >
                        ...
                        >
                          <JCButton
                            buttonType={ButtonTypes.MoreSeriesOutlineBold}
                            onPress={() => actions.changeSeries(index)}
                          >
                            Learn More
                          </JCButton>
            ...
            {moreVideosLength - this.state.numberOfVideos > 0 ? (
              <ListItem style={{ borderBottomWidth: 0 }}>
                <JCButton
                  buttonType={ButtonTypes.Solid}
                  onPress={() => this.handleMoreVideos(moreVideosLength)}
                >
                  Load More Videos
                </JCButton>
              </ListItem>
    ...
    return (
      ...
      >
        <Container style={this.styles.style.resourceContentLeftContainer}>
              ...
              ? state.resourceData.resources.items[state.currentResource].series.items
                  ...
                  .map((series, index: number) => {
                    ...
                    return (
                      <Card
                        key={series.id}
                        style={this.styles.style.resourceContentCurrentSeriesCard}
                      >
                        {state.isEditable ? (
                          <CardItem>
                            <JCButton buttonType={ButtonTypes.TransparentNoPadding} onPress={null}>
          ...
          <Container
            style={{
              height: (this.state.cardHeight * this.state.numberOfVideos) / this.state.rowLength,
              width: "100%",
            }}
          >
            {state.resourceData.resources.items[state.currentResource].series.items
              ...
              .map((series, index: number) => {
                temp.push(series)
                if (
                  index + 1 === moreVideosLength &&
                  this.state.numberOfVideos - moreVideosLength > 0
                ) {
                      ...
                      {temp.map((series2, index2) => {
                        if (series2 === "dummy") {
                          return (
                            <Card
                              key={series2 + index2}
                              style={this.styles.style.resourceContentMoreSeriesCardDummy}
                            ></Card>
                          )
                        }
                        ...
                        return (
                          <Card
                            key={series2.id}
                            style={this.styles.style.resourceContentMoreSeriesCard}
                          >
                            {state.isEditable ? (
                              <CardItem>
                                <JCButton
                                ...
                                <JCButton
                                  buttonType={ButtonTypes.TransparentNoPadding}
                                  onPress={() => {
                                    actions.changeSeries(index + index2 + offset)
                                  }}
                                >
                                  {" "}
                                  <Ionicons
                            ...
                            >
                              <CardItem
                                style={this.styles.style.resourceContentMoreSeriesIframeContainer}
                              >
                                  ...
                                  <Image
                                    ...
                                    source={{
                                      uri: series2.episodes.items[firstEpisodeIndex]?.videoPreview
                                        ? "https://img.youtube.com/vi/" +
                                          series2.episodes.items[
                                            firstEpisodeIndex
                                          ].videoPreview.replace("https://youtu.be/", "") +
                                          "/maxresdefault.jpg"
                                        : "no.jpg",
                                    }}
                                  />
                              ...
                              >
                                <JCButton
                                  buttonType={ButtonTypes.MoreSeriesOutlineBold}
                                  onPress={() => actions.changeSeries(index + index2 + offset)}
                                >
                                  Learn More
                                </JCButton>
                              </CardItem>
                            ...
                            {state.isEditable ? (
                              <CardItem>
                                ...
                                <JCButton
                                  buttonType={ButtonTypes.TransparentNoPadding}
                                  onPress={() => {
                                    actions.changeSeries(index + index2 + 4 - this.state.rowLength)
                                  }}
                                >
                                  {" "}
                                  <Ionicons
                                ...
                                <JCButton
                                  ...
                                  onPress={() => {
                                    actions.deleteSeries(
                                      state.currentResource,
                                      index + index2 + 4 - this.state.rowLength
                                    )
                                  }}
                                >
                                  <Ionicons
  ...
  renderEpisodesMobile(state: ResourceState, actions: ResourceActions): React.ReactNode {
    const series =
            ...
            <ListItem style={{ flexDirection: "column", borderBottomWidth: 0 }}>
              {state.isEditable ? (
                <EditableText
                  ...
                  onChange={(val) => {
                    actions.updateSeries(state.currentResource, state.currentSeries, "title", val)
                  }}
                  multiline={false}
                  ...
                  return (
                    ...
                    >
                      ...
                      <CardItem style={{ width: "100%", paddingRight: 0, paddingLeft: 0 }}>
                        <Container
                          style={this.styles.style.resourceContentEpisodesCardInnerContainer}
                        >
                          {index === 0 ? (
                            <Image
                              accessible={true}
                          ...
                          {episode.videoLowRes || episode.videoHiRes ? (
                            ...
                            >
                              <Text style={this.styles.style.resourceContentEpisodesEpisodeTitle}>
                                Video
                              </Text>
                              <CardItem
                                style={{
                      ...
                      <CardItem style={{ width: "100%", paddingRight: 0, paddingLeft: 0 }}>
                        ...
                        >
                          <TouchableOpacity
                            ...
                            onPress={() => {
                              !state.isEditable ? actions.changeEpisode(index) : null
                            }}
                          >
                            <Image
                              accessible={true}
        ...
        <Container style={this.styles.style.resourceContentEpisodeMainContainer}>
          <Container style={this.styles.style.resourceContentEpisodeLeftContainer}>
            ...
            <EditableText
              key={this.generateKey(state) + "2"}
              onChange={(val) => {
                actions.updateSeries(state.currentResource, state.currentSeries, "description", val)
              }}
              multiline={true}
              inputStyle={this.styles.style.resourceContentEpisodesDescription}
              textStyle={this.styles.style.resourceContentEpisodesDescription}
                ...
                {state.isEditable ? (
                  <CardItem>
                    ...
                    <JCButton
                      buttonType={ButtonTypes.TransparentNoPadding}
                      onPress={() => {
                        actions.deleteEpisode(state.currentResource, state.currentSeries, index)
                      }}
                    >
                      <Ionicons size={24} name="ios-trash" style={this.styles.style.icon} />
                    </JCButton>
  ...
  renderEpisode(state: ResourceState, actions: ResourceActions): React.ReactNode {
    const series =
      state.resourceData.resources.items[state.currentResource].series.items[state.currentSeries]
    const episode =
      state.resourceData.resources.items[state.currentResource].series.items[state.currentSeries]
      ...
      <ResourceContent.Consumer>
        {({ resourceState, resourceActions }) => {
          ...
          if (resourceState.currentSeries != null)
            ...
            else return this.renderEpisodes(resourceState, resourceActions)
