...
class CourseDetailImpl extends JCComponent<Props, State> {
  ...
  getMonth(week, item, lesson) {
    if (item.time && item.tz)
      if (moment.tz(item.time, item.tz).format("MMM DD") == "Invalid date") return ""
      else return moment.tz(item.time, item.tz).format("MMM DD")
    else if (moment.tz(week.date, week.tz).add(lesson, "days").format("MMM DD") == "Invalid date")
      return ""
    else return moment.tz(week.date, week.tz).add(lesson, "days").format("MMM DD")
  }
  getDay(week, item, lesson) {
    if (item.time && item.tz)
      if (moment.tz(item.time, item.tz).format("H:mm") == "Invalid date") return ""
      else
        return (
    ...
    else if (moment.tz(week.date, week.tz).add(lesson, "days").format("H:mm") == "Invalid date")
      return ""
    else
      return (
        moment.tz(week.date, week.tz).add(lesson, "days").format("H:mm") +
        " " +
    ...
    return (
      <Container style={this.styles.style.courseDetailLeftSide}>
        ...
        <Container style={this.styles.style.courseDetailYoutube}>
          <Container style={{ flex: 0.9, height: "auto" }}>
            ...
            <Text style={this.styles.style.courseDetailLessonText}>
              Lesson {state.activeLesson + 1} - {lesson.name}
            </Text>
          </Container>
          <Image
        ...
        <Container style={{ width: "100%" }}>
          ...
          <EditableRichText
            onChange={(val) => {
              actions.updateLesson(state.activeWeek, state.activeLesson, "description", val)
            }}
      ...
      >
        ...
        <Container style={this.styles.style.courseZoomMainContainer}>
          <Container style={this.styles.style.courseDetailSidebarTop}>
            ...
            {lesson.zoomURL && lesson.zoomURL != "" ? (
              <JCButton
                buttonType={ButtonTypes.CourseZoom}
                onPress={() => {
                  this.navigate(lesson.zoomURL)
                }}
              >
                Join Zoom Meeting
              </JCButton>
            ...
            {lesson.zoomRecording && lesson.zoomRecording != "" ? (
              <JCButton
                buttonType={ButtonTypes.CourseZoom}
                onPress={() => {
                  this.navigate(lesson.zoomRecording)
                }}
              >
      ...
      >
        ...
        <Container>
          ...
          <EditableCourseAssignment
            actions={actions}
            assignmentId={actions.getLessonById(lesson.courseLessonResponseId).id}
  ...
  renderLessonDetails(state: CourseState, actions: any, week: any): React.ReactNode {
    if (week?.lessons) {
      ...
      return state.activeLesson != null ? this.renderLessonType(state, actions, week, lesson) : null
    } else {
      return null
    }
  }
