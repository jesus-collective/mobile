...
async function groupMemberByGroup(groupID) {
  try {
    ...
    const recipients = json.data.groupMemberByGroup.items.map((item) => { return item.userID })
    return recipients
  }
  catch (json) {
    console.log(json)
...
function generateMessage(html, text, name, type) {
  const body_text = "You have received a " + type + " message on Jesus Collective from " + name + "\r\n" +
    text
  ...
  const body_html = `<html>
    ...
    <body>
      ...
      <p>You have received a `+ type + ` message on Jesus Collective from ` + name + `</p>` +
    html
...
async function emailRouter(html, text, fromInfo, messageRoomID, recipientInfo) {
  if (messageRoomID.startsWith("group") && (recipientInfo.alertConfig == null || recipientInfo.alertConfig.emailGroupMessage)) {
  ...
  else if (messageRoomID.startsWith("event") && (recipientInfo.alertConfig == null || recipientInfo.alertConfig.emailEventMessage)) {
  ...
  else if (messageRoomID.startsWith("org") && (recipientInfo.alertConfig == null || recipientInfo.alertConfig.emailOrgMessage)) {
  ...
  else if (messageRoomID.startsWith("resource") && (recipientInfo.alertConfig == null || recipientInfo.alertConfig.emailResourceMessage)) {
  ...
  else if (messageRoomID.startsWith("course") && (recipientInfo.alertConfig == null || recipientInfo.alertConfig.emailCourseMessage)) {
